...
class ApiController extends Controller
{
    ...
    {
        if ($this->verifyToken($request->bearerToken()) == true) {
    ...
    public function htmlPay(Request $request)
    {
        ...
        if (in_array($request->currency, $currency)) {
            ...
            $cc = Business::wheresecret_key($request->bearerToken())->count();
            if ($cc == 1) {
                $mode = 1;
                $user = Business::wheresecret_key($request->bearerToken())->first();
            } else {
                $test = Business::wheretest_secret_key($request->bearerToken())->count();
                if ($test == 1) {
                    $mode = 0;
                    $user = Business::wheretest_secret_key($request->bearerToken())->first();
                } else {
                    $data['title'] = 'Error Message';
    ...
    public function popupPay(Request $request)
    {
        ...
        $used = Exttransfer::wheretx_ref($request->tx_ref)->whereuser_id($this->getUser($request->bearerToken())->id)->count();
        $user = User::find($this->getUser($request->bearerToken())->id);
        if ($request->bearerToken() != $user->business()->test_secret_key && $request->bearerToken() != $user->business()->secret_key) {
            return response()->json(['message' => 'Invalid secret key', 'status' => 'failed', 'data' => null], 400);
        } else {
            if ($request->bearerToken() == $user->business()->test_secret_key) {
                $mode = 0;
            } else {
                $mode = 1;
            }
        ...
        if ($this->getUser($request->bearerToken())->status == 0) {
            if ($used == 0) {
                ...
                $sav->mode = $mode;
                $sav->save();
                return response()->json(['message' => 'Payment link created', 'status' => 'success', 'data' => $sav], 201);
            } else {
                return response()->json(['message' => 'User can\'t receive payments', 'status' => 'failed', 'data' => null], 400);
            }
        } else {
                    ...
                    if ($curl->error) {
                        ...
                        if (array_key_exists('meta', (array)$curl->response)) {
                            ...
                            if ($response->data->status == "successful") {
                                ...
                                $balance = Balance::whereuser_id($sav->receiver_id)->wherebusiness_id($link->business_id)->wherecountry_id($sav->currency)->first();
                                if ($sav->pending == 1) {
                                    if ($sav->client == 0) {
