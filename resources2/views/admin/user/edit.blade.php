        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        @if($client->kyc_status!=null)
                        <p>{{__('Line 1')}}: {{$client->line_1}}</p>
                        <p>{{__('Line 2')}}: {{$client->line_2}}</p>
                        <p>{{__('Country')}}: {{$client->getCountry()->name}}</p>
                        @if($client->state!=null)
                        <p>{{__('State')}}: {{$client->myState()->name}}</p>
                        @endif
                        @if($client->city!=null)
                        <p>{{__('City')}}: {{$client->myCity()->name}}</p>
                        @endif
                        <p>{{__('Postal code')}}: {{$client->postal_code}}</p>
                        <p>{{__('Gender')}}: {{$client->gender}}</p>
                        <a href="{{asset('asset/profile/'.$client->proof_of_address)}}">{{__('Proof of Address')}}</a><br><br>
                        <p>{{__('Full name')}}: {{$client->first_name}} {{$client->last_name}}</p>
                        <p>{{__('DOB')}}: {{$client->b_day}}/{{$client->b_month}}/{{$client->b_year}}</p>
                        <p>{{__('ID Document')}}: {{$client->doc_type}}</p>
                        @if($client->document!=null)
                        <a href="{{asset('asset/profile/'.$client->document)}}">{{__('Identity Document')}}</a></br></br>
                        @endif
                        <p class="mb-2">{{__('Compliance Status')}}: {{$client->kyc_status}}</p>
                        @if($client->kyc_status=="PROCESSING")
                        <a class="btn btn-sm btn-neutral" href="{{route('admin.approve.kyc', ['id'=>$client->id])}}"><i class="fal fa-check"></i> {{__('Approve')}}</a>
                        <a class="btn btn-sm btn-danger" href="{{route('admin.reject.kyc', ['id'=>$client->id])}}"><i class="fal fa-ban"></i> {{__('Reject')}}</a>
                        <a class='btn btn-sm btn-primary' data-toggle="modal" data-target="#decline" href=""><i class="fal fa-ban"></i> {{__('Resubmit Compliance')}}</a>
                        @elseif($client->kyc_status=="APPROVED")
                        <a class="btn btn-sm btn-danger" href="{{route('admin.reject.kyc', ['id'=>$client->id])}}"><i class="fal fa-ban"></i> {{__('Reject')}}</a>
                        <a class='btn btn-sm btn-primary' data-toggle="modal" data-target="#decline" href=""><i class="fal fa-ban"></i> {{__('Resubmit Compliance')}}</a> 
                        @elseif($client->kyc_status=="DECLINED")
                        <a class="btn btn-sm btn-neutral" href="{{route('admin.approve.kyc', ['id'=>$client->id])}}"><i class="fal fa-check"></i> {{__('Approve')}}</a>
                        <a class='btn btn-sm btn-primary' data-toggle="modal" data-target="#decline" href=""><i class="fal fa-ban"></i> {{__('Resubmit Compliance')}}</a>                       
                        @elseif($client->kyc_status=="RESUBMIT")
                        <a class="btn btn-sm btn-neutral" href="{{route('admin.approve.kyc', ['id'=>$client->id])}}"><i class="fal fa-check"></i> {{__('Approve')}}</a>
                        <a class="btn btn-sm btn-danger" href="{{route('admin.reject.kyc', ['id'=>$client->id])}}"><i class="fal fa-ban"></i> {{__('Reject')}}</a>
                        <a class='btn btn-sm btn-primary' data-toggle="modal" data-target="#decline" href=""><i class="fal fa-ban"></i> {{__('Resubmit Compliance')}}</a>
                        @endif
                            ...
                            <div class="modal-dialog modal- modal-dialog-centered modal-md" role="document">
                                <div class="modal-content">
                                    <div class="modal-body">
                                        <form action="{{route('admin.resubmit.kyc', ['id'=>$client->id])}}" method="post">
                                            @csrf
                                            <div class="form-group row">
                                                <div class="col-lg-12">
