...
<body>
  <!-- Sidenav -->
  <nav class="sidenav navbar navbar-vertical fixed-left navbar-expand-xs navbar-dark" style="background-color:{{$set->default_color}};" id="sidenav-main">
          ...
          <ul class="navbar-nav mb-3">
            ...
            <li class="nav-item">
              <a class="nav-link @if(route('admin.transactions.all')==url()->current()) active @endif" href="{{route('admin.transactions.all')}}">
                ...
                <span class="nav-link-text">{{__('Payment & API Transactions')}}</span>
              </a>
            </li>
            @endif
            @if($admin->vcard==1)
